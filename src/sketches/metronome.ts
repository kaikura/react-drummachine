        ...
        const loop = (time: number) => {
          ...
          for(let i=0;i<this.num;i++){
              if (i===0){ this.start_meaure_tick.trigger(time)
               //metronome
          }else{   this.tick.trigger(time + i*Tone.Time((this.den + "n")).toSeconds()) }
          }
        //DO DRAWINGS HERE
    ...
    createLoop_2 = () => {
        const loop = (time: number) => {
          ...
          for(let i=0;i<this.num_2;i++){
              if (i===0){ this.start_meaure_tick_2.trigger(time)
               //metronome
          }else{   this.tick_2.trigger(time + i*Tone.Time((this.den_2 + "n")).toSeconds()) }
          }
        //DO DRAWINGS HERE
